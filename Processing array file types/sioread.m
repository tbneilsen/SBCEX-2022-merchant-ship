...
%           if npi == 0 then read all samples
...
%   or generate channel list if channels == 0
...
% Loop over the desired channels
 for J=1:length(channels)
    ...
    for R = 1:totalrec
      ...
      [zz,countRead] = fread(fid,ptrec,type);
      if ( (countRead ~= rl/sl) )
           disp(sprintf(' record R: %4d   returned short read %4d of %4d',R,countRead,rl/sl))
           x(1:p2-p1+1,J) = tmpx(pp1:pp1+p2-p1);
           return
      end
      
      if ( isempty(zz))
           disp(sprintf(' record R: %4d   returned empty read',R))
           x(1:p2-p1+1,J) = tmpx(pp1:pp1+p2-p1);
           return
      end
      
      tmpx(count:count+ptrec-1) = zz;
